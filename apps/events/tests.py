...
class EventCommandTests(CommandTestCase):
    ...
    def test_create_events_default(self):
        ...
        if contracts < 10:
            self.assertEqual(
                out,
                f"Maximum events possible: {contracts}\nCreating {contracts} event(s)...\n",
            )
...
class EventListTests(CustomCRMTestCase):
    ...
    def test_manager_create_event(self):
        ...
        response = test_client.post(
            self.event_list_url, self.event_create_data, format="json"
        )
    ...
    def test_sales_get_event_list(self):
        ...
        for item in response.data:
            self.assertEqual(
                Event.objects.get(contract=item["contract"]).contract.sales_contact,
                user,
            )
    ...
    def test_sales_create_event(self):
        ...
        self.assertFalse(
            Event.objects.get(contract=response.data["contract"]).event_status
        )
    ...
    def test_sales_post_event_incomplete_data(self):
        ...
        test_client = self.get_token_auth_client(user)
        response = test_client.post(
            self.event_list_url, data={"name": "test"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class EventDetailTests(CustomCRMTestCase):
    ...
    def test_manager_get_event_detail(self):
        ...
        for event_id in self.get_id_list(Event.objects.all()):
            response = test_client.get(
                reverse(self.event_detail_url, kwargs={"pk": event_id})
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_sales_update_event_contract(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json(), {"detail": "Cannot change the related contract."}
        )
...
class EventModelTests(CustomCRMTestCase):
    def test_str_event(self):
        ...
        self.assertEqual(
            str(event), "Event #1 : Miller, James | Date : 2022-03-06 (UPCOMING)"
        )
        event = Event.objects.get(id=3)
        self.assertEqual(
            str(event), "Event #3 : Dupont, Jean | Date : 2022-11-17 (COMPLETED)"
        )
