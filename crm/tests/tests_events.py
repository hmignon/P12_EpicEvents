...
class EventTests(CustomAPITestCase):
    ...
    def test_post_event_incomplete_data(self):
    ...
    def test_update_finished_event(self):
        """Sales and support not allowed to update event if finished"""
        user = User.objects.get(username='test_sales')
        test_client = self.get_token_auth_client(user)
        data = {
            'contract': 4,
    ...
    def test_update_event_contract(self):
        """Sales and support not allowed to update contract related to an event"""
        user = User.objects.get(username='test_sales')
        test_client = self.get_token_auth_client(user)
        contract = Contract.objects.create(
            client=Client.objects.get(id=1), amount=123.45, payment_due='2022-06-09', status=True
